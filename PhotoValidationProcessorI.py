...
BUCKET_NAME = "myorganization-images"

# configuration for photo validation in business logic
...
def lambda_handler(event, context):
    ...
    #Extract File Name from PUT Event
    current_file_name = extract_file_name(event)
    
    #Call to Rekognition DetectFaces API
    detect_faces_response = detect_faces(current_file_name)

    #Extracting face details we want for our business logic and their confidences
    #Values we want are values in FACE_DETAILS_TRESHHOLDS
    face_details = extract_face_details(detect_faces_response)
    
    #Evaluating values and thresholds to determine PASS/FAIL
